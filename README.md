...
apk在apk目录下，点击下载最新版本
工程脚本源码在src目录下
* 蓝奏云下载:[点击下载](https://fusong.lanzous.com/b01by2zji
)________密码:g19s
## 注意事项
* 请在使用qq机器人功能是一定要在开始脚本之前授予软件无障碍和通知栏权限，否则将会程序报错。您也可以下载普通版(vbate)解决此问题。
## 目前已测机型
 |机型|版本|对应版本|更新时间|
|  ----  | ----  | ----   |  ----   |
 ...
 * 安卓版本7以上
## 部分手机型号不兼容的解决办法
* 方法一(推荐):在vmos虚拟机中运行，解决机型不兼容的问题(注意要版本大于7.0)，具体使用步骤如上(注意要导入所要监控的软件)。而且在手机息屏的情况也能正常工作。强烈推荐！！！
...
### 第三方支付监听软件
#### 相关介绍:
* 目前只监听支付宝和微信支付，具体后端支付系统源码在另外的仓库pay中，需要的下载即可
...
* qq刷图
#### qq机器人
* 无需第三方qq机器人框架，完全拖关于自己手中，让您账号更安全。
* 注意:系统页面不能停留在qq页面，并且确保qq应用没有加上应用锁！！！！否则无法使用qq机器人功能
##### 调用接口:
|模式|分类|说明|
|  ----  | ----   |  ----   |
  ...
  |小号机器人|略|将自己的小号作为专门的qq机器人，触发关键词和默认模式关键词希望，注意作为qq小号机器人的账号只能允许添加人，不能让作为qq小号机器人的小号加任何群，只做为服务小号使用|
...
|开头关键词|触发接口|说明|例子|
|  ----    | ----   |  ----   | ---- |
|默认|图灵机器人接口|输入开头非关键词默认调用图灵机器人接口，如果无需要您可以关闭|***|
|查题|触发调用查题接口|该开头关键词可以在软件中自定义|查题测试|
  ...
  |翻译|触发翻译接口|仅支持中英文，后接翻译文本|翻译China|
  |抖音|触发抖音热点接口|输入抖音即可，不需要后接词|抖音|
###### 演示1:
...
## 2020.12.19-添加qq机器人功能，并且添加了图灵机器人接口api等查询接口，直接在软件中开启即可
## 2020.12.19-添加查题接口尾部自定义广告，增强接口拓展性
## 2020.12.20-添加qq小号机器人，将自己的小号作为专门的机器人
## 2020.12.20-添加qq小号机器人自动通知被添加好友脚本，并且能自定义好友前缀
## 2020.12.22-添加qq小号机器人被添加后自动回复一条消息。修复程序执行错误机制，让程序增强自我修复能力。
## 2020.12.23-添加翻译接口，
## 2020.12.23-添加抖音热点接口
## 2020.12.31-添加图片文字识别接口
## 2021.1.13-添加微信支付监听
## 2021.9.4- 去除无用接口，替换自主题库接口 -------
